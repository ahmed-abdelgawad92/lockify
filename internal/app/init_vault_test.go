...
func TestInitializeVaultUseCase_Execute_Success(t *testing.T) {
	...
	assert.Equal(t, envTest, vault.Meta.Env, fmt.Sprintf("Execute() returned vault with env %q, want %q", vault.Meta.Env, envTest))
	assert.Equal(t, fingerprintTest, vault.Meta.FingerPrint, fmt.Sprintf("Execute() returned vault with fingerprint %q, want %q", vault.Meta.FingerPrint, fingerprintTest))
	assert.Equal(t, saltTest, vault.Meta.Salt, fmt.Sprintf("Execute() returned vault with salt %q, want %q", vault.Meta.Salt, saltTest))
}
